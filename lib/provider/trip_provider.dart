...
class TripFormNotifier extends _$TripFormNotifier {
  ...
  void updateStartDate(DateTime date) {
    ...
    state = AsyncData(_generateDailyNotesIfNeeded(updatedTrip, _currentState.dailyNotes));
  }
  ...
  // 일차별 상태 생성 유틸
  TripFormState _generateDailyNotesIfNeeded(TripModel trip, List<DailyNoteModel> currentNotes) {
    ...
    final List<DailyNoteModel> newDailyNotes = List.generate(totalDays, (index) {
      ...
      return currentNotes.firstWhere(
        ...
        orElse: () => DailyNoteModel(id: null, tripId: trip.id, dayCount: dayCount, comment: ''),
      );
    });
  ...
  Future<void> addImageFromGallery(XFile pickedFile) async {
    ...
    final comment = TripCommentModel(
      id: null,
      tripId: _currentModel.id, // 기존 _currentModel getter 유지 혹은 _currentState.trip.id 사용
      path: destPath,
      comment: '',
    );
...
// 💡 [추가 포인트 3]: 무효화(invalidate) 순서 정돈 및 디테일 새로고침 강제화
      ...
      state = AsyncData(TripFormState(
        ...
        dailyNotes: dailyNotes.map((n) => n.copyWith(tripId: savedTripId)).toList(),
      ));
    } catch (e) {
  ...
  Future<void> saveDailyNote({required int dayCount, required String comment}) async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      final existingNote = currentNotes.firstWhere(
        (note) => note.dayCount == dayCount,
        orElse: () => DailyNoteModel(id: null, tripId: tripId, dayCount: dayCount, comment: ''),
      );
      ...
      if (existingNote.id == null) {
        ...
        await _repository.updateDailyNote(existingNote.copyWith(comment: comment));
      }
